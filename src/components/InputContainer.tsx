        ...
        <div className={'flex flex-col w-full p-4'}>
            ...
            <form action={(formData) => handleAction(formData)}>
                <div className={'flex justify-between'}>
                    <input
                        ...
                        value={value}
                        className={`w-6/7 border-2 rounded-t-lg ${searchItems.length <= 0 ? 'rounded-b-lg' : ''} p-3 border-gray-200 outline-none focus:border-primary`}
                    />
            ...
            {searchItems.length > 0 &&
                <div className={'p-2 border-2 border-t-0  rounded-b-lg w-6/7 border-gray-200'}>
                    {searchItems.map((item) => <p onClick={() => handleSearchSuggestionClick(item)} className={'cursor-pointer hover:text-shadow-lg'} key={item}>{item}</p>)}
                </div>
            }
